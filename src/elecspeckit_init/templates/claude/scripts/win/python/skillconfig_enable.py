#!/usr/bin/env python3
"""
skillconfig_enable.py - 启用指定的 Skill

功能:
- 更新 skill_config.json 中 enabled: true
- 重命名 (DISABLED)SKILL.md → SKILL.md

用法:
    python skillconfig_enable.py <skill_name> [--config CONFIG_FILE] [--skills-dir SKILLS_DIR]
...
def rename_skill_file(skills_dir: Path, skill_name: str, enable: bool = True) -> None:
    ...
    Args:
        ...
        skill_name: Skill 名称
        enable: True=启用 (DISABLED)SKILL.md→SKILL.md, False=禁用 SKILL.md→(DISABLED)SKILL.md
    """
...
def enable_skill(skill_name: str, config_path: str = None, skills_dir: str = None) -> int:
    """
    启用 Skill
    ...
    try:
        ...
        try:
            rename_skill_file(skills_path, skill_name, enable=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="启用指定的 ElecSpeckit Skill")
    ...
    return enable_skill(args.skill_name, args.config, args.skills_dir)
